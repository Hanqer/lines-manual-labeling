...
void addLabel(const LabImage& data)
{
    ...
    std::string abs_path_copy = "" + abs_path;

    std::string filename = abs_path_copy.replace(abs_path_copy.end()-4, abs_path_copy.end(), ".txt");
    qDebug() << std::string(data.abs_path).data();
    fout.open(filename);
